...
This means that if we perform an addition of 1 on a pointer of type `int *`, the address location will increase by 4 for each step (which is `sizeof(int)`). Similarly, for a `double *` type pointer, the location will increase by 8 (which is `sizeof(double)`). 
...
## The difference between `*(ptr+1)` and `*(ptr++)`
...
After *(++ptr) = 20: arr[3] = {20, 2, 3}
After: ptr = 000000000061FE10
Before: ptr = 000000000061FE0C
After *(ptr+1) = 20: arr[3] = {20, 20, 3}
